...
import com.google.gdbe.client.data.DocumentReference;
import com.google.gdbe.client.data.GDataService;
import com.google.gdbe.client.data.GDataServiceException;
import com.google.gdbe.server.models.AuthenticationToken;
...
public class GDataServiceImpl extends RemoteServiceServlet implements
  ...
  public String getDocumentContents(String resourceId) throws GDataServiceException {
    ...
    DocsService svc = new DocsService("gdbe-0.12");
    svc.setAuthSubToken(token.getToken());
    String contentUri = "http://docs.google.com/feeds/download/documents/Export?exportFormat=txt&docID=" + resourceId;
    try {
  ...
  public boolean setDocumentContents(String documentId, String etag, String contents) throws GDataServiceException {
    ...
    String editMediaUri = "http://docs.google.com/feeds/default/media/document%3A" + documentId;
    try {
  ...
  public boolean deleteDocument(String documentId, String etag) throws GDataServiceException {
    ...
    DocsService svc = new DocsService("gdbe-0.12");
    svc.setAuthSubToken(token.getToken());
    String documentUri = "http://docs.google.com/feeds/default/private/full/document%3A" + documentId;
    svc.getRequestFactory().setHeader("If-Match", etag);
    try {
  ...
  public DocumentReference createDocument(String title, String contents) throws GDataServiceException {
    ...
    DocumentEntry entry;
    try {
      entry = svc.insert(new URL("http://docs.google.com/feeds/default/private/full"), newDocument);
    } catch (Exception e) {
      throw new GDataServiceException(e.getMessage());
    }
    return getDocumentReference(entry);
  ...
  public DocumentReference[] getDocuments(boolean starredOnly) throws GDataServiceException {
    ...
    try {
      feed = svc.getFeed(new URL("http://docs.google.com/feeds/default/private/full/-/document" + (starredOnly ? "/starred" : "")), DocumentListFeed.class);
   ...
   * @throws GDataServiceException
   */
  public String[] getFolders() throws GDataServiceException {
    AuthenticationToken token = getToken();
    if (token == null) return null;
    ArrayList<String> folders = new ArrayList<String>();
    DocsService svc = new DocsService("gdbe-0.12", "http", "gdocs-base-editor.appspot.com");
    svc.setAuthSubToken(token.getToken());
    DocumentListFeed feed;
    try {
      feed = svc.getFeed(new URL("http://docs.google.com/feeds/default/private/full/-/folder"), DocumentListFeed.class);
    } catch (Exception e) {
      throw new GDataServiceException(e.getMessage());
    }
    for (DocumentListEntry entry : feed.getEntries()) {
      folders.add(entry.getTitle().getPlainText());
    }
  ...
  public String logout() throws GDataServiceException {
    ...
    if (token != null) {
      ...
      UserService userService = UserServiceFactory.getUserService();
      return userService.createLogoutURL("http://gdocs-base-editor.appspot.com");
   ...
   * Since these characters are lost, use this function to encode them.
