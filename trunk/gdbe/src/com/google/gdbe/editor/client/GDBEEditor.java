...
import com.google.gdbe.client.data.GDataServiceAsync;
...
import com.google.gdbe.client.events.CommandEventListener;
 ...
 */
public class GDBEEditor implements EntryPoint, CommandEventListener {
   ...
   */
  private void showError(Throwable error, CommandEvent command, CommandEvent alternate){
    ...
    switch (e.getCommand()) {
      ...
      case GENERIC_LOAD_EXISTING_DOCUMENT:
        ...
        gdataService.getDocument(e.getParameters()[0],
            new AsyncCallback<DocumentReference>() {
          public void onFailure(Throwable caught) {
            showError(caught, e, new CommandEvent(errorDialog, CommandEvent.Command.GENERIC_LOAD_NEW_DOCUMENT));
          }
          public void onSuccess(DocumentReference result) {
            setDocument(result);
            clearStatus();
